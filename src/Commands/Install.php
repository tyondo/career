...
namespace Tyondo\Career\Commands;
...
class Install extends BaseCommand
{
     ...
     * @var string
     */
    protected $signature = 'career:install {--y|y : Skip question?} {--views : Also publish tyondo career views.} {--f|force : Overwrite existing files.}';
     ...
     * @var string
     */
    protected $description = 'Tyondo career install wizard';
    ...
    public function handle()
    {
            ...
            $this->comment(PHP_EOL.'Welcome to the Tyondo career Install Wizard! You\'ll be up and running in no time...');

            try {
                $this->comment(PHP_EOL.'Publishing assets...');
                // Publish config files...
                Artisan::call('career:publish:config', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(1);
                ...
                Artisan::call('career:publish:assets', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(2);
                ...
                Artisan::call('career:publish:packages', [
                    '--y' => true,
                    '--force' => false,
                ]);
                $this->progress(3);
                ...
                // Publish view files...
                if ($withViews) {
                    Artisan::call('career:publish:views', [
                        '--y' => true,
                        '--force' => $force,
                    ]);
                }
                ...
                $this->info(PHP_EOL.'Adding career routes to routes/web.php');
                $this->progress(5);
                File::append(
